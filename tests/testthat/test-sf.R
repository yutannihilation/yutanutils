...
test_that("st_add_coordinates() works", {
  expect_equal(st_add_coordinates(points_sf),
               sf::st_sf(value = c(10, 20), geometry = points_sfc,
                         X = 1:2, Y = 1:2))
  expect_equal(st_add_coordinates(points_sf, col_names = c("x", "y")),
               sf::st_sf(value = c(10, 20), geometry = points_sfc,
                         x = 1:2, y = 1:2))
  # the length of col_names is wrong
  expect_error(st_add_coordinates(points_sf, col_names = c("x")))

  # sf_add_coordinates() accepts POINT
  suppressWarnings(x <- st_add_coordinates(polygon_sf))
